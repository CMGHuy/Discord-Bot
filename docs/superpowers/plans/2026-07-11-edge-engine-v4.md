...
> - Task E9 (Growth-path tracker) done: `growth_path()` log-scale progress function verified accurate against golden numbers; `get_balance_history_points()` adapter correctly used the real `"ts"` key (not the brief's placeholder `"at"`), matching the E2/E6/E7 pattern of briefs assuming wrong data shapes. Review found two Important gaps beyond the brief's own scope — no test coverage for the new adapter, and `growth_command`'s appended line always compared progress against a hardcoded 10x regardless of the user's requested `!growth <target>` — both fixed by threading `target` through `_collect_stats` into `growth_path`. That fix itself introduced a new crash path (`target<=1` blows up `growth_path`'s log math with ZeroDivisionError/ValueError), caught by re-review and closed with a validation guard at the `growth_command` boundary plus 4 regression tests. Final state: 787 passed, 54 skipped, +1 known pre-existing unrelated wall-clock failure, carried forward.
> - Task E10 (Phase E0 checkpoint) done: full suite green (787 passed, 54 skipped, +1 known pre-existing unrelated wall-clock failure in `test_trade_monitor_wiring.py`, carried forward) and `python -m py_compile` clean across `bot.py`, `admin_ui.py`, and every `swingbot/**/*.py`. Live-account smoke:
>   - `growth_table()` sensitivity grid matches hand-derived expectations exactly (e.g. 1% risk / +0.10R expectancy -> 2303 trades-to-10x, matching this plan's own stated example).
>   - Live Monte Carlo smoke used the real accessor `TradeLog().get_trades(limit=None)` (not `all_trades()` as the brief's placeholder guessed, and not the private `_trades` attribute) filtered on `r_multiple is not None` -> `N = 0` on this dev environment's current `data/trades.json` (no closed trades logged yet here), so `simulate()` correctly reported "need >=10 closed trades" rather than running on an empty/degenerate sample -- this is the expected degrade-gracefully path, not a bug.
> - Task E11 (Friction model in the backtest) done: `swingbot/core/edge/frictions.py` (`apply_frictions`/`commission_r`) verified accurate against golden numbers; `run_backtest(..., frictions=True)` default-on for the v1 exit-model loop only — the `exit_model="v2"` branch (live Plan Engine's already-validated `plan_engine.simulate_exit`) is deliberately untouched, confirmed by checking that every later `frictions=` reference in this plan only ever calls the v1-routed functions. Implementer correctly deviated from the brief's literal code placement (moved the friction-adjusted `risk_per_share` recompute to strictly after the v2 branch's `continue`) since the brief's own snippet, applied literally, would have silently leaked friction arithmetic into v2's `return_pct` — caught before it could break `test_v2_scale_out_keeps_classification_and_expectancy`. 5 existing tests asserting exact v1 economics got `frictions=False` added with a stated reason each. TRAIN re-baseline: all 11 strategies still PASS with frictions on, 0.033–0.047R haircut per strategy as predicted (`docs/superpowers/results/2026-07-22-edge-baseline.md`, started here per plan, finalized at E22). Reviewer independently re-verified the v2-untouched claim via diff and the frictions=False bit-identical claim by tracing the arithmetic. Full suite 791 passed, 54 skipped, +1 known pre-existing unrelated wall-clock failure, carried forward.
> - Task E12 (Liquidity screen) done: `liquidity_ok`/`liquidity_reason` (20-day avg dollar volume + min price floor) verified against the brief's golden numbers. Wired into the live scan loop's `_sync_run_scan` deliberately AFTER `update_open_trades`/`_check_near_close` and BEFORE the new-signal horizon loop — the brief's own snippet placement (right after the df fetch) would have silently stopped SL/TP monitoring on already-open paper trades whenever their ticker dipped below the liquidity floor; caught and fixed, backed by a real non-mocked integration regression test. Also wired into BOTH of `scripts/run_backtest_range.py`'s ticker loops (`main()`'s per-strategy grid AND `run_scenario_mode()`'s confluence replay) — the latter was a real gap the original implementer missed. Process note: the originally dispatched implementer subagent died mid-task (no report/commit) after writing a complete but untested diff; the controller took over, found and fixed the `run_scenario_mode` gap, verified, and committed. Full suite 796 passed, 54 skipped, +1 known pre-existing unrelated wall-clock failure, carried forward. Reviewer approved with zero blocking findings (one Minor NaN-volume edge case noted, inherited from the brief's own reference code, deferred to E16).
> - Task E13 (Universe files + S&P 500 builder) done: `load`/`universe_symbols`/`sector_map` appended to `universe.py` (validated+deduped, `"+"`-concatenation, unknown/missing -> `[]`), `etfs.json` (17 hand-written rows), `scripts/build_universe.py`, `SCAN_UNIVERSE` config Field (default `watchlist`, verified genuine no-op today), wired into the real `tickers = load_watchlist()` call site in `_sync_run_scan`. Data-sourcing: controller pre-scoped `sp500_top150.json` OUT of this task before dispatch — it needs a full-S&P-500 OHLCV cache that doesn't exist (`fetch_backtest_data.py` has no `--universe` flag; live-fetching ~500 tickers was judged an inappropriate long/network-heavy operation for this task) — the `--top` code path is implemented but unexercised, honestly disclosed in the script's own docstring, matching E11's disclosed-gap precedent. `sp500.json` (503 symbols) built from a real live Wikipedia fetch, parsed via `pandas.read_html` for accuracy on a 500-row table (confirmed to introduce zero new runtime dependency — an ad-hoc one-off scrape step, not committed/imported code). Reviewer independently re-verified data integrity by script (0 dupes, 0 empty sectors, 0 ETF leakage into the stock file, correct BRK.B→BRK-B/dual-class normalization) rather than trusting the report. Full suite 799 passed, 54 skipped, +1 known pre-existing unrelated wall-clock failure, carried forward.
> - Task E14 (Index/ETF plan support) done: `is_etf()` appended to `universe.py` (process-cached from `etfs`+`sp500` files). Brief's module/function names (`market_events.py`/`days_to_earnings`) don't exist in this repo — pre-resolved to the real `swingbot/core/events.py` `get_next_earnings_date()`/`earnings_within_window()` before dispatch; short-circuit placed at the top of `get_next_earnings_date` (the one real network-calling function), transitively protecting `earnings_within_window` too. Implementer caught and fixed their own test bug mid-task: a raise-based monkeypatch would have been silently swallowed by the function's existing `try/except Exception`, making the test pass even without the fix — rewrote as a call-recording spy, verified airtight by disabling the short-circuit and confirming red. Reviewer independently reproduced that exact mutation check live. `build_strategy_plan` (plan_engine.py) used as the real end-to-end builder (brief's guessed `trade_plan.build_trade_plan` turned out to be a deprecated shim delegating to the same function) — confirmed zero `is_etf` references anywhere in plan construction, proving ETF symbols get no special-casing. Full suite 802 passed, 54 skipped, +1 known pre-existing unrelated wall-clock failure, carried forward. Approved, zero findings.
> - Task E15 (Incremental data cache) done: `update_cache()` appended to `data_store.py` (last-cached-date read, fetch-only-newer via injectable `fetch_fn`, atomic `.tmp`+`os.replace` write, dedup `keep="last"` favoring fresh rows). Two real brief gaps resolved before dispatch: `data_store.py` had no `log` object at all (added, matching sibling-module convention); and this codebase has TWO parallel OHLCV cache subsystems (`backtest_cache.py`/`data/backtest_cache/`, used by today's existing grid/backtest scripts, vs `data_store.py`/`market_data/`, which every LATER Edge-plan task E16/E19+ actually depends on) that the brief conflated — resolved by implementing `update_cache` in `data_store.py` as specified and wiring `fetch_backtest_data.py`'s new `--universe` flag as a purely ADDITIVE path, with the cache-location disconnect honestly disclosed in 3 places rather than silently building an orphaned feature or expanding scope into `backtest_cache.py`/`run_backtest_range.py` (neither touched, confirmed via diff). Implementer independently caught and fixed a real `TypeError` in the brief's own sample (`.date()` compared against a `DatetimeIndex` raises `TypeError` on pandas 2.3.3) — reviewer reproduced the exact failure standalone before confirming the fix. Full suite 804 passed, 54 skipped, +1 known pre-existing unrelated wall-clock failure, carried forward. Approved, zero blocking findings.
> - Task E16 (Data-quality validator) done: `data_quality_issues()` (frozen-feed, unadjusted-split, non-positive price, calendar gap) appended to `universe.py` as a sibling to E12's `liquidity_reason`, wired into the SAME three spots (engine.py's `_sync_run_scan` preserving E12's existing-trade-monitoring invariant; both `run_backtest_range.py` ticker loops). Two real brief defects pre-resolved before dispatch: the brief's `validate_data.py` sample imported the wrong `DATA_DIR` (app-data dir, not the OHLCV cache) and used a glob that doesn't match `data_store.py`'s real nested layout. Also resolved the E15-established two-cache fork the same way: `validate_data.py` is dual-mode (default validates the real, populated `data/backtest_cache/`; `--universe NAME` validates the still-empty `market_data/`), mirroring E15's `fetch_backtest_data.py` precedent so the brief's own "run it once over the real cache" step is actually meaningful. Real run over `data/backtest_cache/`: 8/77 symbols flagged (ASTS/BKNG/HIMS/QBTS frozen closes; CRWV/HOOD/QBTS/SHOP/SOFI split-like jumps), reported honestly rather than hand-edited. Full suite 808 passed, 54 skipped, +1 known pre-existing unrelated wall-clock failure (plus one unrelated load-sensitive perf-test flake, confirmed passing on isolated rerun), carried forward. Approved, zero blocking findings.
...
- Task E31 (Data-driven stops from MAE distributions) done: `swingbot/core/edge/stops.py` (`MIN_SAMPLE=40`, `MAE_CUSHION_R=0.15`, `CLAMP=(0.8, 1.3)`, `mae_informed_stop_mult`) + `DATA_DRIVEN_STOPS_ENABLED` config Field (default false) + plan-engine wiring. The distribution math is the brief's, verified correct against all four of its own golden numbers. The brief's wiring snippet had **three real defects**, each found by grepping the symbol rather than trusting the plan. (1) *It would have contaminated the backtest.* `_atr_plan` is the SHARED sizing source for both `build_strategy_plan` (live) and `backtest._trade_plan_at` (v1 and v2 loops) -- the whole "live behavior equals backtested behavior by construction" property rests on that. A `journal.entries()` read placed inside it, as the snippet implies, would silently price 2020 backtest trades off today's live journal: unreproducible results and a lookahead leak. Resolved with dependency injection, the pattern this codebase already uses for exactly this problem (`confidence.score_confidence`'s injected `track_record`, E24's injected `regimes`): `_atr_plan(..., stop_mult=None)` and `build_strategy_plan(..., stop_mult=None)` are trailing defaulted params, the journal lookup happens only at the `build_strategy_plan` boundary, and `backtest._trade_plan_at` is untouched (asserted by a test) so E33's fold harness must pass its own fold-train-derived value explicitly. (2) *`plan.quality_breakdown.append(f"MAE-informed stop x{mult:.2f}")` would have crashed every v2 alert.* That list is strictly `(name, points)` tuples: `embeds.py:382` and `views.py:139` both render it as `{pts:+d}`, and `plan_to_dict` flattens each row with `list(row)` -- a bare string breaks both renderers and explodes into 23 single characters in the persisted plan JSON. The applied factor got its own typed `TradePlanV2.stop_mult_applied` field instead (trailing default, so every already-persisted plan still loads -- tested), which is also what E33/E40 actually need: an after-the-fact audit trail of which plans were sized with it. (3) *The append was placed before `_apply_quality`, which replaces `plan.quality_breakdown` wholesale* -- it would have been silently wiped for every plan built with `quality_inputs` even had the type been right. Scope call: applied to the ATR-multiple path only. Fibonacci/Elliott/Support-Resistance stops sit behind real structure (a fib swing, a wave-2 low, an S/R shelf) and scaling those slides the stop off the structure it exists to hide behind -- a different, unvalidated idea; asserted by a signature test so a later task can't quietly extend it. Scaling `risk_distance` rather than the stop price preserves R:R exactly (the same distance feeds stop and target), so the frozen `STRATEGY_RR_OVERRIDE` table and the 0.30 floor don't move, and the horizon's `max_risk_pct` cap still binds after the scaling (both tested). Failure paths: a corrupt/unreadable journal logs and returns None rather than breaking plan construction; flag-off never opens the journal at all (tested with a fail-on-call stub). Live state: `data/journal.json` holds 0 entries on this machine, so even flipping the flag on is a no-op until 40+ winners per strategy are journaled. Tests: `tests/test_edge_stops.py`, 18 (brief's 5 + losers/None-MAE exclusion + 12 wiring/regression). Full suite 874 passed / 54 skipped (+1 known pre-existing wall-clock failure), py_compile clean across 102 files.
...
- Task E37 (Composite entry-quality score v2) done — **with its Step-3 decile-audit half NOT executed; see the blocker below.** `rs_points`/`mtf_points`/`breadth_points`/`candle_points`/`gap_penalty` added to `quality.py` (brief's formulas transcribed after hand-checking all nine of its golden assertions) and wired into `score_plan` as **keyword-only params with inert defaults**. The brief's own test is self-inconsistent (imports `compute_quality`, calls `compute_quality_with_edge`); neither exists — the real entry point is `score_plan(*, direction, regime, htf_bias, confluence_count, volume_ratio, atr_pct, trigger_distance_pct, badge_status)`, extended in place rather than via the brief's suggested wrapper, since defaulted keyword-only params ripple to nobody. A component row is appended **only when its input was actually supplied**: the breakdown is rendered verbatim in embeds, so a row must mean a real reading, not an absent one that happened to score zero. Bit-identical-without-edge-inputs is asserted directly, which matters because the A/B/C tier thresholds and the registry badges were both built on the current scoring. Context worth recording: **the live scan never passes `quality_inputs` at all** (`attach_plan_v2` calls `build_confluence_plan` without it, so `_apply_quality` returns early and every live v2 plan carries `quality_score=0, tier="C"`), so quality scoring today is exercised only by the offline audit harness and the tests — this task cannot change live behavior, and wiring the scan to supply these inputs is not in this task's file list. **BLOCKED — Step 3's "re-run the decile audit and commit the table":** three of the five new inputs cannot be produced by that harness. It scores each ticker in isolation from the CSV cache with `regime=None, htf_bias=None, confluence_count=0`; `rs_percentile` and `breadth` are universe-wide readings it has no universe for, and `gap_fragile` needs E17's gate wired into a per-trade context that doesn't exist. Only `mtf` and `candle_quality` are computable there. Running the harness unchanged would reproduce the existing table byte-for-byte (score_plan is bit-identical without edge inputs) — evidence of no regression, not evidence that the new components rank-order outcomes, which is what the brief asks the audit to show. Deferred to E43's ablation harness / the E39 fold engine, which have the universe context this needs. Tests: 5 (brief's component goldens + None-handling + bit-identical + append/clamp + a worst-case floor). Full suite 908 passed / 54 skipped (+1 known pre-existing wall-clock failure), py_compile clean across 102 files.
- Task E38 (Pyramiding rules) done — **and its headline risk invariant is FALSE; see below.** `maybe_pyramid(plan, price)` added to `plan_manager.py`, `PYRAMIDING_ENABLED` config Field (default false), `TradePlanV2.pyramid_add` field, and a flag-gated `_step_partial` hook emitting a `pyramid_add` PlanEvent at most once per plan. Shape corrections: `TradePlanV2` has **no `risk_per_share`** field (the brief's stand-in invents one) — 1R is `abs(entry_price - stop_loss)`, which is safe to read at PARTIAL precisely because the breakeven move writes `working_stop` and never mutates `stop_loss` (verified in `_step_active`); `_step_partial` already uses that same idiom. **THE BRIEF'S RISK INVARIANT DOES NOT HOLD, and its own test asserts a bound its own numbers violate.** The brief claims the add's stop at the original entry "caps total position risk at ≤ the original 1R at every moment", that a normal stop-out "nets ≥ breakeven on the whole campaign", and that a gap costs at most 1R of new downside. At this project's frozen R:R — TP1 lands at ~0.35–0.5R, +0.5R in the brief's own example — all three fail, because banking half at TP1 (+0.5 × 0.5R) cannot pay for an add that risks 0.5 × 1R: (a) a **clean** stop-out at the add's own stop, no gap at all, turns a +0.25R campaign into **−0.25R**; (b) the brief's own gap example (everything filling at 97) costs **1.25R** of new downside, so its `assert total_r >= no_pyramid_r - 1.0` is false; (c) even a gap no worse than the ORIGINAL stop leaves the campaign at **−1.25R**, worse than the un-pyramided plan's own −1R; (d) whole-campaign break-even sits at 100.5, above the add's stop, so the add can never exit at its stop for free. All four numbers are pinned in `test_the_briefs_claimed_risk_invariant_does_not_hold` so a redesign starts from facts, and the warning is repeated in the function docstring and the config help text. The component still ships as specified — pure, flag-gated OFF — because it is the rule the folds are meant to judge, but **it needs redesigning (smaller add, later trigger, or a TP1 beyond 1R) before it is worth enabling**, and that is not this task's call to make. The manager hook is a SUGGESTION only: no leg realized, no stop moved, no status change, asserted by test. Tests: 6. Full suite 914 passed / 54 skipped (+1 known pre-existing wall-clock failure), py_compile clean across 102 files.
- Task E39 (Walk-forward engine) done: `swingbot/core/backtest_wf.py` (`ANCHORED_FOLDS`, `GATE_*` constants, `_apply_overrides`, `_guarded`, `_symbols_for_folds`, `_frame_for`, `_default_run`, `run_folds`, `gate`) + `scripts/wf_run.py` + `tests/test_wf_engine.py` (14). Fold table and all three gate constants transcribed verbatim and pinned by tests, including the exact boundaries (a −0.05R fold and N=30 both PASS) — changing them silently would invalidate every decision recorded against them; a separate test asserts the burned 2024–2025 window is unreachable from any fold. **`_default_run` as written in the brief was never callable:** `run_backtest_daterange`'s real signature is `(ticker, df, strategy, horizon_key, date_from, date_to, ...)`, and the brief's five positionals bound `start` to `horizon_key` and left `date_to` missing → `TypeError`. It also has no horizon dimension at all. Fixed with an explicit `horizons` loop and pinned by a regression test that asserts the actual call args. Two further corrections: `watchlist.get_watchlist` doesn't exist (real name `load_watchlist`), and `t.r_multiple` is `float | None`, so `np.mean` over an unfiltered list would have poisoned every fold with NaN — now filtered, with a test. **Exit-model correction (methodology, not typo):** `run_backtest_daterange` had no `exit_model`/`scale_out` pass-through, so the brief's harness would have measured folds under the **v1** exit model while the E22 friction-adjusted baseline every component is judged against was measured under **v2 + scale-out** — the deltas would not have been comparable. Added the three pass-through params to `run_backtest_daterange` with defaults identical to `run_backtest`'s (asserted by test, so no existing caller moves) and `_default_run` now requests v2 + scale-out + frictions explicitly. `_guarded` was factored out so an INJECTED `run_fn` gets the same try/finally config restore as the default one: a component run that raises must not leave `config` mutated for the next component's folds, which is tested directly. Live smoke: `python scripts/wf_run.py --strategy RSI --horizon 4w` runs end-to-end against the real `market_data/` cache (78 watchlist symbols, all with frames) and correctly returns **FAIL** on N=2/1/4 folds — thin folds are exactly what the pre-registered N≥30 rule is there to reject. **Runtime note for E33:** the full sweep is every symbol × 11 strategies × 10 horizons × 2 legs × 3 folds; at `SCAN_UNIVERSE=sp500` that is 503 symbols and firmly in the hours category — run it deliberately, chunked, never casually. Full suite 928 passed / 54 skipped (+1 known pre-existing wall-clock failure), py_compile clean across 119 files (swingbot + scripts).
- **E38 FIXED (follow-up to the falsified invariant).** The rule's add fraction is no longer the fixed 0.5 it was specified with — `pyramid_add_fraction(plan)` derives it from the plan's own banked TP1 (`tp1_fraction × |tp1 − entry| / 1R`, capped at 0.50, and no suggestion at all below 0.05). Derivation: at a clean stop-out the campaign is `banked − f·R`, so `f ≤ tp1_fraction × (tp1 − entry) / R` is exactly the breakeven condition. Three invariants now hold and are pinned by tests: (1) a clean stop-out nets **≥ breakeven** (was −0.25R); (2) a gap all the way to the plan's ORIGINAL stop leaves the campaign at −0.75R, **better than the −1R that plan was always willing to lose**; (3) a gap beyond the original stop is unbounded — stated honestly, since no stop-based rule bounds a gap, though the derived fraction more than halves the damage versus a fixed 0.5 add. The old fixed-0.5 numbers are kept as a regression test so nobody re-hardcodes it. Uses the plan's own numbers rather than the R:R constant, so it stays correct across `STRATEGY_RR_OVERRIDE` and for a TP1 re-priced by E32.
- **E39 FIXED (two harness defects found while preparing E33).** (a) `_default_run` ran with `tp2_mode="none"`, under which the backtest never calls `build_level_map` — so **every level-sourced component was invisible** and would have scored a meaningless 0.0000 delta while burning its one-shot pre-registration. Verified empirically (flipping `AVWAP_LEVELS_ENABLED` is bit-identical under `"none"`, moves expectancy 0.225949→0.226346 under `"levels"`). Now `"levels"`, which is also `run_backtest_range.py`'s own default and therefore what the E22 baseline used. (b) `_frame_for` read `market_data/`, which starts 2018-06 — exactly the folds' anchor, leaving indicators no warm-up; on AAPL the same strategies yield 13 trades from `backtest_cache/` vs 1 from `market_data/` over identical windows, so fold N was being starved by the data source alone. Switched to `data/backtest_cache/`, which is also the cache the E22 baseline was measured on.
- Task E33 (Fold-tune the Phase-E2 filter set) **RUNNING** — `scripts/wf_components.py` launched over the full watchlist (78 symbols × 11 strategies × 10 horizons, v2 + scale-out + `tp2_mode=levels` + frictions), writing `docs/superpowers/results/2026-07-26-edge-folds.md` / `.json` / `.log`. The script backtests each (symbol, strategy, horizon) **once per leg** and slices trades into folds by `entry_date`, instead of re-running the whole backtest per fold as `run_backtest_daterange` does — identical numbers, a third of the work.
  - **Only 2 of 6 flags are registered**, and that is the honest count, not a shortcut: `AVWAP_LEVELS_ENABLED` and `VOLUME_PROFILE_NODES_ENABLED` are the only ones the backtest can observe. The other four are **bit-identical in this harness by construction** and are documented in `INERT_COMPONENTS` rather than run, because registering them would score 0.0000 and spend their single pre-registered shot on a test that could not have detected anything: `DATA_DRIVEN_STOPS_ENABLED` reaches only `build_strategy_plan` while the backtest sizes through `_trade_plan_at`; `REGIME_GATES_ENABLED` needs a `regimes` series `run_backtest → _vectorized_entries → entries_for` never supplies (and `REGIME_ALLOW` is empty anyway); `PYRAMIDING_ENABLED` lives in the live plan manager, which `simulate_exit` has no notion of; `EARNINGS_BLACKOUT_DAYS` was never wired into any scan or backtest path.
...
- Task E41 (Permutation test) done: `scripts/permutation_test.py` (`permuted_expectancies`/`p_value`/CLI) + `ENTRY_SHIFT` hook in `backtest.py` (module-level default 0, no-op when unset). Executed ahead of E33 (pure functions + synthetic-data tests, no dependency on E33's real fold output).
- Task E42 (Parameter plateau check) done: `PLATEAU_TOLERANCE_R` + `plateau_report()` appended to `backtest_wf.py`. Scope narrowed at dispatch — `wf_components.py` has no gridded-parameter-sweep concept today, so only the pure function + test landed; wiring waits for a future gridded-component task. Executed ahead of E33 alongside E41.
- Task E33 (Fold-tune the Phase-E2 filter set) **complete** — the background sweep launched above finished (253.4 min). Both tested components **FAIL** decisively: `AVWAP_LEVELS_ENABLED` pooled delta −0.0001R (0/3 improving folds), `VOLUME_PROFILE_NODES_ENABLED` −0.0002R (1/3 improving, needs ≥2). Neither adopted; `adopted_components.json` committed as `{}`. **Phase E2 closes with ZERO adopted components** — not a burned pre-registration (nothing untested was ever run against real data), but it means every downstream "adopted system" task (E40's double-eval, E41/E42's real invocation, E43's ablation, E51's portfolio replay) operates over an empty set. The prerequisite for a real future grid is the deferred fix list above (thread `stop_mult`/`tp2_r` through `run_backtest`, thread a `regimes` series through `entries_for`, wire pyramiding into `simulate_exit`, wire E18's earnings blackout).
- Task E40's blocked sub-step now resolved by E33's actual result: the fold-passing list is empty, so there's nothing left to build (no code change needed).
- Tasks E41/E42 confirmed N/A for real invocation: permutation/plateau evidence only applies to adopted components, and none were adopted. Both tools remain correct and tested for future use.
- Task E43 (Ablation harness) done: `scripts/ablation.py` (leave-one-out over `adopted_components.json`), verbatim per plan. Not invoked against a live fold sweep — with the adopted set empty, `run_folds({})` would cost ~76 min of real compute to confirm a mathematically guaranteed +0.0000R with zero ablation rows; documented rather than burned, same reasoning as `INERT_COMPONENTS`.
- Task E44 (Phase E2/E3 checkpoint) done: full suite green against the E33 commit (3 failures triaged: the 1 documented pre-existing wall-clock failure; 1 flaky subprocess test that passed in isolation under shared-machine load; 1 untracked test file belonging to a concurrent session's own in-progress admin-risk-panel work, not touched). Evidence pack closed: fold doc complete, permutation/plateau/ablation correctly N/A, `adopted_components.json` committed empty, flags unchanged (off).
- Task E45 (Drawdown throttle ladder) done: `DD_LADDER`/`RESUME_DD_PCT` frozen constants + `drawdown_pct`/`current_throttle` in new `swingbot/core/edge/throttle.py`, `DD_THROTTLE_ENABLED` config Field. Pure function ships; wiring (`throttle_state.json`, live `effective_risk_pct` call site, paused-alert labeling) deferred to a later task, matching this plan's established score-ships/wiring-deferred pattern (E7/E8/E17/E18/E30/E35).
- Task E46 (Loss-streak damper + kill combination) done: `STREAK_TRIGGER`/`STREAK_MULT`/`STREAK_RECOVERY_WINS`/`COMBINED_FLOOR` + `streak_multiplier()`/`combined_throttle()` appended to `throttle.py`. Pure append, no wiring, matches E45.
- Task E47 (Kill switch, wired live) done: `kill_state()`/`set_kill()`/`check_kill_triggers()` in `throttle.py`; scan-loop wiring in `engine.py` (data-quality-fail fraction aggregated safely across `map_tickers`' parallel workers); `!killswitch` admin-gated Discord command. Also added REAL sizing zeroing when heat/cluster blocks engage (E7/E8's blocks previously only labeled alerts without changing displayed numbers) — a genuinely new, reviewer-confirmed correct addition beyond the brief's own file list.
- Task E48 (Position recycling) done: `RECYCLE_PROGRESS_R` + `recycle_candidates()` appended to `plan_manager.py`. Pure function, tick-wiring deferred (same pattern).
- Task E49 (Sector heat cap) done: `sector_heat()`/`sector_check()` in `heat.py` + `SECTOR_HEAT_CAP_PCT` config Field. Pure function, wiring deferred.
- Task E50 (Portfolio-level backtest replay) done: `portfolio_replay()` appended to `backtest_wf.py` (heat-cap-aware chronological replay) + `collect_portfolio_signals()`/`scripts/wf_run.py --portfolio` (designed by the controller against real `BacktestTrade` fields, since the brief's own CLI text was too vague to transcribe) — including a real near-duplicate dedup port from the live scanner's `has_similar_open_trade`, found missing in fix round 1.
- Task E52 (`!portfolio` command) done: `portfolio_report()` in `growth.py` + `_collect_portfolio_state()` (designed by the controller against every real signature — heat, sector heat, correlation clusters, throttle, kill state, growth path — since the brief only described it in prose). Fix round 1 wrapped the one uncovered sub-collector in the same fail-safe try/except the function's own contract requires.
- Task E53 (Weekly risk report) done: `weekly_risk_report()` in `growth.py` + a new Sunday-only branch in `retrospective.py`. `heat_utilization_pct`/`throttle_activations` are honest stubs (no telemetry/event log exists yet for either) rather than fabricated data. Fix round 1 restored a cluster-size filter dropped from a duplicated E52 loop, which had been reporting spurious singleton "clusters" from tickers self-correlating at 1.0.
- Task E54 (Admin risk panel) done: `/risk` route + kill-switch POST + `risk.html`, reusing `_collect_portfolio_state` from E52. Implemented and committed by a concurrent session sharing this working tree; retroactively verified (tests passing) rather than re-done.
- Task E55 (Sizing-mode shadow comparison) done: `shadow_sizing` stamped on every logged trade + `scripts/sizing_shadow_report.py`. Also implemented and committed by a concurrent session; retroactively verified.
- Task E51 (Portfolio replay of the adopted system) done: `portfolio_replay()` extended with `r_multiples_taken` (chronological R-multiples of every trade actually opened — the brief's Step 2 assumed a `data/replay_r_sequence.json` producer that didn't exist anywhere in the repo; this closes that gap) + `wf_run.py --r-sequence-json`. Real replay run against the full watchlist: baseline system (zero adopted components per E33), `final_multiple` 2.196x over ~3.0 years (~30% CAGR), max DD 9.88%, 61.3% of raw signals skipped by heat/sector caps. `docs/superpowers/results/2026-07-26-edge-portfolio.md` is now **the honest growth-expectation source** the plan's own File Structure section designates — quote `!growth` ETAs from it, not from per-signal expectancy. Fix round 1 corrected one transcribed `p_ruin` cell.
- Task E56 (Phase E3 checkpoint) done for its automatable parts: full suite green, E51's portfolio doc committed, this Progress block updated. **Step 2 (live Discord smoke test — `!portfolio`, `!killswitch on/off`, admin `/risk` page) requires a human in a real test channel and could not be executed by an agentic worker; deferred to `.superpowers/sdd/2026-07-11-edge-engine-v4/DEFERRED-MANUAL-VERIFICATION.md` for the user to run.** Phase E3 (E45–E56) otherwise fully closed.
- Task E57 (`decision_chart.py` skeleton) done, opening Phase E4: 3×4 GridSpec one-pager, main candle panel with plan-level lines (chart-never-crashes invariant honored throughout), `decision_panels.py` dispatcher with E58/E60/E65 stubs. Byte-for-byte transcription of the brief's own complete code.
- Tasks E68/E69/E70/E71/E73 done in parallel, in the separate `swingbot/core/charts/portfolio_charts.py` (new file — zero overlap with E57–67's `decision_chart.py`/`decision_panels.py`): heat treemap, correlation heatmap, Monte Carlo fan (`ruin.simulate` gains `return_paths`, wired live into `!growth` — attaches when the account has ≥10 closed trades, strictly additive), growth-path chart, fold-evidence chart. **Task E72 (regime timeline) is blocked, not skipped carelessly:** it imports `REGIME_SHADE` from `decision_chart.py`, which doesn't exist yet — that's E61's own deliverable, still ahead of where the decision-chart chain currently stands. Revisit once E61 lands.
- **Full detail for every task above** (exact numbers, fix-round history, reviewer findings) lives in the plan's own SDD ledger: `.superpowers/sdd/2026-07-11-edge-engine-v4/progress.md` — this Progress block is now a summary pointer to it, not the sole record, to keep this already-358KB file from growing further.
- **Next:** Task E58 (Weekly context panel) continues on the `decision_chart.py` front; **Task E72** (blocked on E61) and **E74–E76** (need the full decision-chart panel set plus live human smoke-testing) are what's left on the `portfolio_charts.py` front.
- **Task E92 RETIRED 2026-08-05 — superseded by plan v8 Task V24** (executed as V24 Step 1, whose whole purpose is that the one-shot window cannot be fired twice from two plans). E92 pre-registered the single permitted reuse of the 2024–2025 window in `docs/superpowers/results/2026-07-27-edge-final-shot.md` (commit `0c0dd9f`, 2026-07-27) but **never fired** — that file had no `## Result` section and a clean `git diff HEAD` when this was checked. Plan v8's V24 now owns that shot, under a window a human-partner directive widened on 2026-08-03 to `1999-01-01..2026-08-03`; because that window **swallows TRAIN**, V24 Step 2 carries a mandatory full / in-sample / out-of-sample decomposition and **adoption reads the out-of-sample row only**. E92's gate table is fitted to E92's narrower window and does not transfer unexamined. The supersession is recorded in the results file itself, not only here, because `wf_run.py --once-guard` reads that file and not this plan. **Retired, not done:** the measurement is now V24's to make, and v8 records that it must not be fired without an explicit human decision — V17 and V52 both ended with an empty adopted set, so there is currently no adopted config for it to validate.
...
- [x] **Step 1: Full suite + `make check` — green.**
- [x] **Step 2: Finalize `docs/superpowers/results/2026-XX-edge-baseline.md`** (started at E11): the frictions-ON, liquidity-screened TRAIN 2018–2023 table per strategy (N, WR, expectancy_r, max DD) — **this is the reference number every Phase-E2 component must beat.** Include the frictions-off deltas and the list of liquidity/data-quality excluded symbols.
- [x] **Step 3: Update the Progress block and commit**
...
- [x] **Step 4: Run `python -m pytest tests/test_edge_stops.py -v` — PASS (5 tests).**
...
- [x] **Step 1: Write the failing test** (append to `tests/test_edge_stops.py`)
...
- [x] **Step 3: Implement** (append to `stops.py`)
...
- [x] **Step 4: Run `python -m pytest tests/test_edge_stops.py -v` — PASS (9 tests).**
...
- [x] **Step 4: Run `python -m pytest tests/test_edge_factors.py -v` + full suite — PASS.**
...
- [x] **Step 2: Run — FAIL (`ImportError: maybe_pyramid`).**

- [x] **Step 3: Implement** (add to `plan_manager.py`)
...
- [x] **Step 4: Run `python -m pytest tests/test_edge_stops.py -v` — PASS. Full suite green (flag off).**
...
- [x] **Step 4: Run `python -m pytest tests/test_wf_engine.py -v` — PASS (6 tests).**
...
### Task E92: The single 2024–2025 shot — **RETIRED 2026-08-05, superseded by plan v8 Task V24**

> **Do not execute the steps below.** E92 and plan v8's **V24 are the same shot** — two plans each reserved "the one permitted reuse" of this window, which is exactly how a one-shot budget gets spent twice by two sessions that never talk to each other. V24 owns it now. E92 never fired (`2026-07-27-edge-final-shot.md` had no `## Result` section and a clean `git diff HEAD` when checked on 2026-08-05); the supersession is recorded **in that results file**, because `wf_run.py --once-guard` reads it and not this plan. V24's window was widened by human-partner directive on 2026-08-03 to `1999-01-01..2026-08-03`, which swallows TRAIN — so Step 2's gate table below, fitted to the narrower window, does **not** transfer unexamined. See the edge-engine-v4 Progress block entry and plan v8 Task V24.
