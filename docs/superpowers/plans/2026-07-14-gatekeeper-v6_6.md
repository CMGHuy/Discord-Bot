...
>
> **Split note:** this is part 6 of 11, extracted verbatim from the pre-split master plan (deleted 2026-07-26, see the part index; the checklist-to-task traceability appendix is in Part 11). Parts execute in numeric order.
...
**Interfaces:** `check_calendar(df_daily, plan, macro_snap) -> CheckResult` — pass when the macro snapshot is fresh (< TTL) and its events section is populated (the bot literally checked the calendar this session); warn when stale; unknown when `MACRO_ENABLED` off.
- [ ] **Step 1: Write the failing test** (append to `tests/test_gate_timing.py`)
...
def check_calendar(df_daily, plan, macro_snap, **ctx) -> CheckResult:
    """Did the bot literally check the calendar this session? This checks
    that we LOOKED at the calendar at all."""
...
Weight table (initial):
  ...
  redflag: fake_breakout 10, dead_cat 10, stop_sweep 8,
           divergence_trap 8, extreme_fade 8, buy_rumor 6,
