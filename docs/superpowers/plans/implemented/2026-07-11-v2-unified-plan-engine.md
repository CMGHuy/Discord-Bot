...
**Spec:** `docs/superpowers/specs/implemented/2026-07-11-v2-unified-plan-engine-design.md` — read it before starting any task.

## Progress

> Updated by the executing session after each phase/task batch. Resume from the first unchecked task.
>
> - **Branch:** `main` (work merged past `feature/plan-engine-v2` long ago)
...
> - **Completed:** Tasks 22–31, 34–38 — 2026-07-17 (git: `8818404` = Task 37, `d665870` = Task 38). NOTE: the earlier claim here that "Tasks 22–38" were done was wrong — 32–33 had never run (no doc/JSON/`--from-json`, registry still carried round-1 v1 numbers); caught and corrected 2026-07-18.
> - **Completed:** Tasks 39–46 (confluence grid FAIL-on-train, gates adopted as honest defaults, VALIDATION FAIL at every horizon, registry WEAK records, scan-engine flag wiring, backtest_confluence retired), 47–55 (quality score + audits), 56–73 (PlanStore/PlanManager lifecycle), 74–84 (Discord UX), 86–87 (shadow logger + parity report) — 2026-07-17 (see `.superpowers/sdd/progress.md` for per-task commits).
> - **Completed 2026-07-18:** Task 32 (exit-v2 VALIDATION single run: 7/11 PASS, ExpR up ~+0.16R median; `2026-07-exit-v2-validation.md`), Task 33 (`--from-json` + registry regenerated with v2 numbers), Task 89 steps 1–2 (`plan_numbers_for_display` cutover funnel + tests; deploy steps NOT done), Task 92 (README/DOCKER/.env/spec docs), Task 93 (pooled report: **VALIDATED pool N=814 WR=84.2% ExpR=+0.259 — spec criterion 2 MET**), Tasks 95–97 (RSI rescue: max_adx=20 range gate → OOS N=30 WR=100.0 ExpR=+0.304 → **VALIDATED**), Tasks 98–100 (RSI Divergence confirmation gate → 0/9 train configs qualify → **REJECTED-ON-TRAIN**, stays WEAK, validation unspent). Deviation notes: RSI-Div reclaim strength measured against the rolling 20-bar range midpoint (plan's discrete swing-mid formula is vacuous for the rolling detector); rescue gates default OFF in `DEFAULT_PARAMS` until their train grid adopts winners (Task 95's "gains {25, True}" interface text read as the hypothesis grid, not instant adoption).
> - **Completed 2026-07-18 (concurrent session):** Tasks 104–106 (Elliott strict wave-2 gate; train winner w2_max_retrace=0.618/dur=0.75; OOS N=75 WR=77.3 ExpR=+0.064 → **FAIL, stays WEAK**; registry patched to the gated numbers).
> - **Remaining:** Task 85 (phase 6 manual smoke — needs test guild), 88 (shadow ≥5 sessions), 89 steps 3–4 (deploy `on`), 90 (enable scale-out+manager), 91 (legacy deletion after a clean week), 94 (phase 7 checkpoint) — all operational, need real calendar time; Tasks 101–103 (MA Ribbon rescue), 107–109 (EMA Crossover rescue), 110 (final report + VERSION bump).
...
**Files:** Modify `README.md`, `DOCKER.md`/`DEPLOY_HETZNER.md` (flag documentation), `docs/superpowers/specs/implemented/2026-07-11-v2-unified-plan-engine-design.md` (mark shipped).
