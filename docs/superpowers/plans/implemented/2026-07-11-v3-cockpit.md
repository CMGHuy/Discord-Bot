...
**Architecture:** Part 1 builds `swingbot/core/analytics/` (metrics, MFE/MAE, 10-dimension aggregation, quality calibration, edge-decay detection, the shared `follow_score`, a lessons journal, a snapshot file, atomic JSON writes). Parts 2 and 3 only *render* what Part 1 computes — no stat is ever computed twice. Written 2026-07-11 assuming `2026-07-11-v2-unified-plan-engine.md` (110 tasks) is fully implemented and merged.
...
> - Plan C: Phase C0 (Tasks C1-C5, foundations — vendored JS, Flask test harness, component CSS, api/pages blueprints) done. Phase C1 (Tasks C6-C13, data endpoints & speed — /api/stats, /api/plans, /api/journal, /api/calibration+/api/registry, ETag 304 polling, gzip, chart.png caching, 500-row dashboard history bound) done. Full suite green (664 passed, 54 skipped) after each task. **Several ground-truth deviations found and fixed along the way** (documented in each task's commit): `swingbot.core.analytics.snapshots.refresh_snapshot()` is void-return, not dict-returning as C6 assumed (api.py wraps it); `PlanStore` already existed from plan-engine-v2 with a different interface than C7 assumed (no new plan_store.py written); `JournalStore` lives at `swingbot.core.analytics.journal`, not `swingbot.analytics.journal`, and its `entries()` has no `limit` kwarg (C8); snapshot's `by.strategy` is a list of StatRow dicts keyed via `"key"`, not a strategy-name-keyed dict (C9). Phase C2 (Plans board UI): C14 (Plans board page — server-rendered ranked board via shared `_plan_rows`/`_ranked_plan_rows` in pages.py, api.py imports it back), C15 (Lifecycle strip — `.lifecycle-strip` status-count cards scoped to today via existing `_is_today_berlin`, one post-review fix round to add real content assertions for the today-scoping gate), C16 (Board filters — status/tier/badge selects + ticker substring input on `_plan_rows`, one post-review fix round for a lifecycle-strip link that dropped the ticker param), C17 (Plan cancel/close actions — `/plans/<id>/cancel`+`/close` routes with state-machine guards and linked-trade closure, one post-review fix round for missing linked-trade-closure test coverage), C18 (Board auto-refresh — `/plans/fragment` ETag route mirroring `/dashboard/fragment`, morphdom auto-refresh preserving filter query params, one post-review fix round to port dashboard.html's INPUT/SELECT morphdom guard so in-progress filter edits survive a poll), and C19 (Plan detail page — `/plans/<id>` with chart/timeline/quality-breakdown/badge-stats/follow-breakdown/linked-trade, `/plans/<id>/chart.png`; controller found and pre-corrected FIVE real bugs in the brief before dispatch — see ledger for detail — then one post-review fix round for a self-inflicted duplicate-CSS-property bug plus two test-coverage gaps) done. **PHASE C2 (Plans board UI) COMPLETE.** Phase C3 (Strategies page) in progress: C20 (Registry provenance table — `/strategies` + `/api/registry` refactored onto a shared `_registry_rows()` helper; controller found and pre-corrected four real bugs — wrong module path, list-vs-dict snapshot shape, unfiltered 22-vs-11 registry records, a stale test value — plus implementer self-found a required test-monkeypatch retarget from the refactor), C21 (Strategy × horizon live win-rate heatmap; controller had to write and supply a corrected `_strategy_horizon_heatmap()` before dispatch since the brief's assumed 2D `stats_by(keys=...)` cross-tab API doesn't exist anywhere in the codebase — real `stats_by` only supports single-dimension grouping; one post-review fix round for a real `None`-win_rate crash on all-manually-closed buckets the corrected code left unguarded), C22 (Drift columns + DECAY alert chips — controller found and pre-corrected three real bugs: a stale `_registry_rows()` base, wrong drift-entry key names that would have made the DECAY chip silently never fire against real production data, and a matching wrong test fixture; review clean, no fix round needed), and C23 (Rolling-WR sparklines — brief verified fully accurate, no corrections needed; one post-review fix round to strengthen tests that only checked tag/class presence into ones asserting real SVG content and rolling-window/scratch-exclusion semantics) done. **PHASE C3 (Strategies page) nearly complete — only C24 remains.** Full suite green (689 passed, 54 skipped) aside from one **pre-existing, unrelated** failure independently confirmed by the controller: `tests/test_trade_monitor_wiring.py::test_flag_on_polls_open_plans` is a wall-clock/expiry-dependent fixture that now fails against today's real date — not a cockpit-v3 regression, flagged separately, not yet fixed. **Known, honestly-documented gap carried forward:** the manual-close-notify queue write for plan transitions (`kind: plan_transition`) is not yet recognized by the real Discord consumer (`notify_closed_trades` in scanning.py/embeds.py) — cancel/close currently does not post to Discord; fixing the consumer is out of C17's file scope and not yet scheduled as its own task.
> - Task C24 (Cross-links — found already implemented/committed on `main` with no ledger entry on 2026-07-21 resume; independently verified via diff read + test run + full suite, reconciled into the ledger) done. **PHASE C3 (Strategies page, C20-C24) COMPLETE.** Phase C4 (Calibration & Journal pages): C25 (Calibration page — controller pre-corrected two real ASSUMED-SHAPE bugs in the plan's own text: tier verdict key is `ok` not `pass`, drift keys are `drift_alert`/`delta_wr` not `decayed`/`delta`; one post-review fix round for a `chart-card`/`chart-title` CSS gap, both classes existed only inside stats.html's local `<style>` block — promoted to the shared stylesheet), C26 (Journal browser — controller pre-corrected two real bugs: wrong `JournalStore` import path, and a non-existent `limit` kwarg on the real `entries()` — fixed via post-call slicing matching the already-shipped `/api/journal` route's own fix for the same issue; review clean), C27 (Journal note editing — brief verified fully accurate against the real `/api/journal/<id>/note` endpoint, no corrections needed; review clean), and C28 (Weekly digest view — controller pre-corrected three real bugs: wrong `insights` import path, wrong assumed zero-arg signatures for `weekly_digest`/`top_lessons` (real signatures need `entries`/`closed`/`today` sourced via the file's own established `TradeLog` pattern, matching the live Discord `!lessons week` command), and a stale diff context predating C26's own `limit` fix; one previously-unflagged full-suite failure (`test_watch_button_toggles_star`) investigated and confirmed a one-off flake via independent re-run, not a regression; review clean) done. **PHASE C4 (Calibration & Journal pages, C25-C28) COMPLETE.**
> - **Phase C5 (Tuning workbench, C29-C37) COMPLETE.** C29 (admin `JobManager`), C30 (`/api/jobs*` routes), C31 (TRAIN-only guardrail, hardened twice against real review/scanner findings — `--flag=value`/non-padded-date bypasses, non-dict `params`/non-numeric `be_trigger`), C32 (`/tuning` current-parameters table), C33 (grid launch form, TRAIN window shown immutably, no date input), C34 (live job-progress poller + recent-jobs list), C35 (`scripts/tune_strategy.py --json` output + results comparison table — fixed a real path-traversal hole in `_load_result` found by an automated security scan, `_JOB_ID_RE` allow-list), C36 (`POST /tuning/propose` writes a proposal file, never auto-applies to `DEFAULT_PARAMS`), C37 (proposal browser + diff view + delete — implementer's own security self-review caught a live, empirically-demonstrated Windows backslash path-traversal/arbitrary-delete in the delete route before the scanner even ran; fixed with the same `_JOB_ID_RE`-style allow-list). Full suite green (725 passed, 54 skipped, +1 known pre-existing unrelated wall-clock failure in `test_trade_monitor_wiring.py`).
> - **Phase C6 (Settings v2, C38-C41) COMPLETE.** C38 (diff-preview confirm modal before save, sensitive fields masked "•••" on both sides — found and fixed a real CSS gap: `.cal-overlay`/`.cal-modal`/`.cal-modal-header` only existed in `stats.html`'s page-local `<style>`, not shared, so a page-local copy was added to `settings.html`; implementer also fixed two real brief/test inconsistencies, both independently re-traced and confirmed by review), C39 (settings_audit.jsonl trail + "Recent changes" panel, sourced from C38's already-masked diff output), C40 (export omits sensitive fields entirely / import accepts them — implementer fixed a real pre-existing UTF-8 encoding bug in `_write_env_text`; task reviewer caught a serious plan-mandated gap where one of the brief's own tests lacked the `admin_app` isolation fixture and could have silently overwritten the real project `.env` — confirmed via cold-import check, fixed, re-verified in true isolation), C41 (client-side-only "Only changed" filter + per-field ↺ reset button, zero Python changes, explicitly carried the `admin_app` fixture forward given C40's history) done. Full suite green throughout (final: 105/105 admin, full suite includes the 1 known pre-existing unrelated wall-clock flake in `test_trade_monitor_wiring.py`).
> - Task C42 (Pedigree chips + runner leg rows on open/closed dashboard tables — zero Python changes, template-only) done. Task C43 (Lifecycle strip + equity sparkline on dashboard) done: controller pre-corrected two real assumed-shape bugs in the brief before implementing — `snap["equity_curve"]` is `{"points": [...], "skipped_n": N}` (the real `metrics.equity_curve` return shape), not a bare list as the brief's test fixture assumed, and each point keys its balance by `"date"`, not `"ts"`; also normalized raw account-balance points to a 0-100 scale before handing them to `_sparkline_svg`, which is hardcoded to assume a 0-100 input (shared with the Strategies page's win-rate sparkline) — passing raw balances (10000+) through unmodified would have plotted every point off the visible canvas. Import path also corrected to the real `swingbot.core.analytics.snapshots` (brief said `swingbot.analytics.snapshots`, the same class of correction made repeatedly earlier in Phase C3/C4). Full suite green (738 total: 737 passed, 54 skipped, +1 known pre-existing unrelated wall-clock failure in `test_trade_monitor_wiring.py::test_flag_on_polls_open_plans`).
> - Task C44 (Performance page v2 — calibration-deciles chart, tier-calibration table, badge-drift table, and the strategy×horizon heatmap added to the top of `/performance`, all sourced from the same analytics snapshot the Calibration/Strategies pages already read; heatmap block extracted into a shared `_heatmap.html` partial, preserving the real `cell.win_rate is none` guard rather than the plan's simplified `cell.n < 5`-only condition) done. Controller pre-corrected the same class of stale-key-name bug already found in C25: tier verdict key is `ok` (three-valued True/False/None), not `pass`; drift row keys are `oos_wr`/`live_wr`/`delta_wr`/`drift_alert`, not `decayed`/`delta` — copied verbatim from the already-shipped, already-correct `calibration.html`. Full suite green (737 passed, 54 skipped, +1 known pre-existing wall-clock failure in `test_trade_monitor_wiring.py`); one additional **environment-flake** failure observed on two separate full-suite runs — a Windows-only `PermissionError` on `plans.json.tmp` → `plans.json` rename in `plan_store.py._save`, hitting a different, unrelated test each run (`test_lifecycle_strip_counts_and_click_filters` once, `test_api_plans_status_filter` another time; both pass standalone) — not caused by this task's changes (neither file touched), not investigated further, flagged here for whoever hits it next.
> - Task C45 (Mobile + polish pass) done: manual responsive audit at 900/640/480px via a live running admin server (chrome-devtools MCP, real project data) across every new cockpit-v3 page — dashboard lifecycle strip/equity card, Performance's new panels, the shared heatmap, Plans/Journal/Tuning tables and filter bars — found zero CSS gaps, confirmed rather than assumed; no `style.css` changes needed. Added the one genuinely missing empty state (Calibration page's tier/drift tables, per the brief) plus a test for it.
> - **Task C46 (Checkpoint — full verification) done. Plan C (Part 3, Admin control center) COMPLETE. All three parts (A, B, C) of cockpit-v3 are now COMPLETE — 115/115 tasks.** Step 1: full suite green (739 passed, 54 skipped, +1 known pre-existing wall-clock failure in `test_trade_monitor_wiring.py`, unrelated to this plan — same one carried forward and documented since Phase C3); `make check`-equivalent (`python -m py_compile` over `bot.py admin_ui.py swingbot/**/*.py`) exits 0. Step 2 (live smoke): **read-only portion done, live-mutation portion deliberately skipped** — by explicit user decision this session (same call as Task B38's Discord walkthrough), rather than performed. Confirmed via a live `python admin_ui.py` run against real project data + chrome-devtools MCP: Dashboard (lifecycle strip, equity sparkline, pedigree chips all render against real data), Plans (board + filters render, empty state correct — no plans exist in this environment), Strategies (registry table, heatmap, badges all render with real OOS numbers), Calibration (deciles chart + tier/drift tables), Journal (empty state), Tuning (current-parameters table), Settings (fields render, bot token genuinely masked). **Not performed** (mutates real production data/state, out of scope for an automated-only pass): cancelling/closing a real plan, launching a real tuning grid, editing/exporting/importing real settings, verifying the `/dashboard/fragment` poll's 304 behavior live (attempted via the browser tool but the MCP's network-request list didn't surface post-load fetch() calls — ETag/304 mechanics are already covered by Task C10/C18's automated tests, not re-verified live). Step 3: added a new "Admin cockpit" section to `README.md` (between "Analytics core" and "Files") covering the five new pages, the job system (logs/results/proposals paths), the TRAIN-only guardrail, and the settings audit trail/masking — every path and behavior claim in it verified against real source (`jobs.py`, `helpers.py`, `pages.py`) before writing, including one self-caught correction: `import_env_text`'s real behavior applies recognized sensitive keys on import (a deliberate credential paste is the whole point) rather than skipping them, contrary to an initial draft sentence. `DOCKER.md` needed no changes (no top-level path changed). Step 4: committed.**
...
Three corrections found by reading `2026-07-11-v2-unified-plan-engine.md` directly (Task 56 for `PlanStore`, Task 70 for the `TradeLog` integration) instead of trusting a one-line paraphrase:
...
```bash
git add tests/test_analytics_perf.py README.md docs/superpowers/plans/implemented/2026-07-11-v3-cockpit.md
...
Update the plan document's Progress block (top of the combined `2026-07-11-v3-cockpit.md` document, not this draft) to read:
...
```bash
git add docs/superpowers/plans/implemented/2026-07-11-v3-cockpit.md
...
- [x] **Step 2: Extract `_heatmap.html`**
...
- [x] **Step 3: `stats_page` context (`app.py`, replacing lines 1095–1102)**
...
- [x] **Step 4: New panels at the top of `stats.html`**
...
- [x] **Step 1: `python -m pytest tests/ -q` + `make check` — green.**
...
```bash
git add docs/superpowers/plans/implemented/2026-07-11-v3-cockpit.md README.md DOCKER.md
