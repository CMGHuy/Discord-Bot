# Gatekeeper v7 - Part 11/11: Ops, governance & wrap-up (+ traceability appendix) (Tasks G197-G216)
...
def test_quota_report_warns_over_80pct(monkeypatch):
    ...
    import swingbot.core.macro.health as health
    monkeypatch.setattr(health, "provider_status", lambda: {
        "treasury": {"calls_today": 1300}})
    report = health.quota_report(now=dt.datetime(2026, 7, 14, 8, 0))
    assert report["treasury"]["projected"] == 3900        # > 80% of 3000 → warn
    assert report["treasury"]["warn"] is True
...
**Files:** Modify `swingbot/commands/scanning.py` (Sunday hook); test `tests/test_gate_audit.py`
...
```text
# --- Gatekeeper / market context (free tier) ---
...
| "Grab rotation/CPI/PPI/treasury before every trade" | G9–G44, G39 (pre-scan), G119, G122, G147–G152 |
