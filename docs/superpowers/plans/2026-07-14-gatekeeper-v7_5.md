...
- [x] **Step 1:** For each of the six skipped checkpoints, restate in one sentence what it was meant to verify (e.g. Task 88: "shadow-mode v2 plans match legacy scenario numbers, 0 invariant violations, ≥80% coverage over ≥5 sessions"). (Done for all six in the doc.)
- [x] **Step 2:** For each, check whether real production data since 2026-07-18 can answer the same question retroactively (e.g. `shadow_parity_report.py` needs actual shadow-mode logs — if the system went straight to `PLAN_ENGINE_V2=on` with no shadow period, this data was never generated and the checkpoint literally cannot be answered from history; say so plainly rather than approximating). (Local `data/` has no real production data — `trades.json` is `[]`, no `journal.json`, `scan_telemetry.jsonl` is test-generated not production. Task 88 is unanswerable in principle (shadow log never generated anywhere); Tasks 85/89/90/94 are answerable in principle but not from this session, which has no access to the remote Hetzner deployment's data.)
- [x] **Step 3:** For every checkpoint you *can* answer retroactively, do so and record the number/verdict. For every one you cannot, write an explicit waiver: what it was, why it can't be verified after the fact, and what weaker evidence (if any) substitutes for it (e.g. "system has run N weeks in production with zero related incidents" is weaker than the original gate but is honest about being weaker). (Full-suite/`make check` verified green for the tasks that included it. All live-verification halves explicitly waived with weaker substitute evidence per task — no pass fabricated.)
- [x] **Step 4: Commit**
