...
> - Phase C5 (Tuning workbench): C29 (admin `JobManager` — subprocess job runner serialized to one job at a time, state in `data/admin_jobs.json`, stale-pid reaping; brief matched plan text exactly, no corrections needed), C30 (`/api/jobs*` routes — `POST /jobs/tune` placeholder argv, `GET /jobs/<id>` status+log_tail, `GET /jobs` recent-20 list; no corrections needed), and C31 (TRAIN-only guardrail — `build_tune_args`/`assert_train_only` are now the sole constructor/gate for tuning argv, wired into both `build_tune_args` and `JobManager.start`; base implementation matched plan text, then two self-directed hardening rounds closed real review findings: `--flag=value` single-token dates and non-zero-padded dates in `assert_train_only`, and non-dict `params`/non-numeric `be_trigger` raising `ValueError` instead of an uncaught `TypeError` in `build_tune_args`) done. Full suite green (711 passed, 54 skipped, +1 known pre-existing unrelated wall-clock failure in `test_trade_monitor_wiring.py`).
> - **Next:** Task C32 (Phase C5, Tuning page — current state)
