...
> - **Completed:** Tasks A1-A31 — **Plan A (Part 1, Analytics & Insight Core) done.** Plan B: Tasks B1-B19 — **Phase B0 (B1-B4), Phase B1 (B5-B8), Phase B2 (B9-B14) done.** Phase B3 (Commands, B15-B25) in progress: B15-B16 (`!liveplans` live ranked board + filters — renamed from `!plans` per user decision; `!plans` itself, in history.py, is untouched), B17 (`!top`, new swingbot/commands/stats.py module, config.DIGEST_MAX_PLANS added), B18-B19 (`!stats` embed + period filters).
> - **Next:** Task B20 (`!lessons`)
