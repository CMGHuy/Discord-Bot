...
**Outcome: no measured edge. VALIDATION not run. NOT merged to `main`.**
...
(default `false`). This code exists only on worktree branch
`worktree-2026-08-16-v36-level-touch-strength` — since it showed no benefit,
it was left there unmerged rather than landed inert on `main`. See Decision.
...
- **VALIDATION: not run.** It is a one-shot per
  ...
  stop here.
- **Not merged to `main`.** Tasks 1–5's code (`find_touches`, `grade_level`,
  `Level.strength`, the tiebreak, the confidence factor) was complete and
  tested, but with no measured benefit there is no reason to land it —
  inert or not, it's the codebase's job to justify a merge, not the merge's
  job to prove itself later. It stays on
  `worktree-2026-08-16-v36-level-touch-strength` for reference or salvage.
  This is a measured "doesn't help here," not a stub.
