...
template:`@if(data();as d){<sb-panel heading="Exit quality"><p class="coverage">{{coverageText()}}</p>@if(unmappedText()){<p class="unmapped">{{unmappedText()}}</p>}<div class="grid"><section><h3>Exit reason mix</h3><sb-donut [slices]="reasons()"/></section><section><h3>Outcome mix</h3><sb-donut [slices]="outcomes()"/></section><section><h3>Exit efficiency — winners only</h3><sb-histogram [bins]="efficiency()"/><p>{{d.efficiency.n}} winner trades · median {{d.efficiency.median ?? '—'}}R</p></section><section><h3>MAE — winners only</h3><sb-histogram [bins]="mae()"/><p>{{d.mae.n}} winner trades · median {{d.mae.median ?? '—'}}R</p></section><section><h3>Disposition</h3><p>{{disposition()}}</p></section><section><h3>MFE versus MAE</h3><sb-scatter [points]="scatter()" xLabel="MAE (R)" yLabel="MFE (R)"/></section></div></sb-panel>}`,
styles:`.coverage{color:var(--text-muted);font-size:var(--text-micro)}.unmapped{color:var(--warn);font-size:var(--text-micro)}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:var(--space-14)}h3{font-size:var(--text-table);margin:0 0 var(--space-8)}p{font-size:var(--text-micro);color:var(--text-muted)}`})
export class ExitQualitySectionComponent {
  ...
  protected readonly unmappedText=computed(()=>{const d=this.data();const rows=d?.unmapped_reasons??[];const reasons=d?.exit_reasons as any[]??[];const total=reasons.reduce((n,r)=>n+(r.n??0),0);const other=reasons.find(r=>r.reason==='other')?.n??0;if(!rows.length||!total||other/total<=.2)return null;return `${Math.round(other/total*100)}% of exits are unmapped: ${rows.map(r=>r.text||'(no reason recorded)').join(', ')}`;});
