...
styles:`.coverage{color:var(--text-muted);font-size:var(--text-micro)}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:var(--space-16)}h3{font-size:var(--text-table);margin:0 0 var(--space-8)}p{font-size:var(--text-micro);color:var(--text-muted)}`})
