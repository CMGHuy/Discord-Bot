...
@Component({ selector: 'sb-pipeline-tab', changeDetection: ChangeDetectionStrategy.OnPush, imports: [Panel, PanelHeader, Histogram, EmptyStateComponent], template: `
  ...
  <sb-panel><sb-panel-header title="Badges" [allTime]="true" />
    @if (badgeBins().length) { <sb-histogram [bins]="badgeBins()" /> } @else { <sb-empty-state title="No badged strategies yet" reason="measured-zero" /> }
  </sb-panel>
