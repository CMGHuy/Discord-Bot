...
`, styles: `.verdict,.panels{display:grid;gap:var(--space-14)}.verdict{grid-template-columns:repeat(auto-fit,minmax(10rem,1fr));margin:0}.verdict div{padding:var(--space-10);border:1px solid var(--border);border-radius:var(--radius)}dt{font-size:var(--text-micro);text-transform:uppercase;letter-spacing:.1em;color:var(--text-faint)}small{font-size:var(--text-micro);color:var(--text-faint)}dd{margin:var(--space-4) 0;color:var(--text);font-variant-numeric:tabular-nums}.panels{grid-template-columns:repeat(2,minmax(0,1fr))}@media(max-width:639px){.panels{grid-template-columns:1fr}}` })
