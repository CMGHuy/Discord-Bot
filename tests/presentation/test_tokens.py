...
def test_accent_ramp_is_monotonic_worse_to_better():
    ...
    assert t.accent_for_level(3).value == 0x9EA2AD
...
def test_outcome_accents_are_the_same_three_colours_as_the_ramp_ends():
    ...
    assert t.accent_for_outcome("scratch").value == 0x9EA2AD


def test_unknown_outcome_is_grey_not_a_crash():
    assert t.accent_for_outcome("").value == 0x9EA2AD
    assert t.accent_for_outcome("garbage").value == 0x9EA2AD


def test_blocked_accent_is_the_neutral_grey():
    assert t.ACCENT_BLOCKED == 0x9EA2AD
