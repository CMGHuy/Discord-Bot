...
def test_requires_auth(client):
    assert_error(client.get("/api/v1/universe/tickers"), "auth", 401)


def test_list_is_empty_initially(watchlist, logged_in):
    assert logged_in.get("/api/v1/universe/tickers").get_json() == {"tickers": []}


def test_list_shape(watchlist, logged_in):
    watchlist(["AAPL"])
    body = logged_in.get("/api/v1/universe/tickers").get_json()
...
def test_add_a_single_ticker(watchlist, logged_in):
    r = logged_in.post("/api/v1/universe/tickers", json={"tickers": ["AAPL"]})
...
def test_writes_land_in_the_isolated_data_dir(watchlist, logged_in):
    ...
    logged_in.post("/api/v1/universe/tickers", json={"tickers": ["ZZZZ"]})
...
def test_one_endpoint_handles_bulk(watchlist, logged_in):
    r = logged_in.post("/api/v1/universe/tickers", json={"tickers": ["AAPL", "MSFT", "NVDA"]})
...
def test_a_pasted_blob_is_accepted(watchlist, logged_in):
    ...
    r = logged_in.post("/api/v1/universe/tickers", json={"tickers": "AAPL, MSFT\nNVDA"})
    assert sorted(r.get_json()["added"]) == ["AAPL", "MSFT", "NVDA"]
...
def test_one_bad_symbol_does_not_fail_the_batch(watchlist, logged_in):
    ...
    r = logged_in.post("/api/v1/universe/tickers",
...
def test_already_present_is_reported_separately_from_added(watchlist, logged_in):
    watchlist(["AAPL"])
    body = logged_in.post("/api/v1/universe/tickers",
...
def test_a_non_list_body_is_rejected(watchlist, logged_in):
    assert_error(logged_in.post("/api/v1/universe/tickers", json={"tickers": 5}),
                 "invalid", 400)


def test_remove_a_ticker(watchlist, logged_in):
    watchlist(["AAPL", "MSFT"])
    r = logged_in.delete("/api/v1/universe/tickers/AAPL")
    assert r.status_code == 200
...
def test_remove_is_case_insensitive(watchlist, logged_in):
    watchlist(["AAPL"])
    assert logged_in.delete("/api/v1/universe/tickers/aapl").status_code == 200


def test_removing_an_absent_ticker_is_404(watchlist, logged_in):
    assert_error(logged_in.delete("/api/v1/universe/tickers/NOPE"), "not_found", 404)
...
def test_trade_counts_are_attached(watchlist, logged_in, tmp_path):
    ...
    row = logged_in.get("/api/v1/universe/tickers").get_json()["tickers"][0]
