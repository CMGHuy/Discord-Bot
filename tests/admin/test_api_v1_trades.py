...
def test_confidence_filter_replaces_the_retired_tier_filter(seed, logged_in):
    ...
    Distinct from `test_confidence_filter` below (which pre-dates v32 and
    covers the same endpoint from a plans-seeded angle) so neither name
    shadows the other and both cases stay collected.
    """
    seed(trades=[
