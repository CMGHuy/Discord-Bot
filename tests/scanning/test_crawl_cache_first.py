...
from swingbot.core.scanning import engine as scan_engine, fetch, runstate
...
def test_stop_request_still_ends_the_crawl_early(monkeypatch, no_network):
    ...
    monkeypatch.setattr(fetch, "_load_cached_daily", lambda t: make_ohlcv([1.0, 2.0]))
    monkeypatch.setattr(runstate, "is_stop_requested", lambda: True)
