...
def _assert_section_failure_is_visible(tmp_path, monkeypatch, caplog, target, section):
    from swingbot.core.tracking import retrospective as retro

    monkeypatch.setattr(retro, "_HISTORY_PATH", str(tmp_path / "history.json"))

    def _boom(*args, **kwargs):
        raise RuntimeError(f"{section} exploded")

    monkeypatch.setattr(target[0], target[1], _boom)
    with caplog.at_level("ERROR", logger="swing-bot.retrospective"):
        messages = retro.build_daily_retrospective([], today=dt.date(2026, 9, 6))

    joined = "\n".join(messages)
    assert "Daily Retrospective" in joined
    assert f"skipped: {section}" in joined.lower()
    assert any(section in record.message.lower() for record in caplog.records)


def test_weekly_risk_failure_adds_degraded_notice(tmp_path, monkeypatch, caplog):
    from swingbot.commands import growth

    _assert_section_failure_is_visible(
        tmp_path, monkeypatch, caplog,
        (growth, "weekly_risk_report"), "weekly risk",
    )


def test_rs_rotation_failure_adds_degraded_notice(tmp_path, monkeypatch, caplog):
    from swingbot.core.edge import factors

    _assert_section_failure_is_visible(
        tmp_path, monkeypatch, caplog,
        (factors, "load_rs_cache"), "rs rotation",
    )


def test_scan_health_failure_adds_degraded_notice(tmp_path, monkeypatch, caplog):
    from swingbot.core.scanning import engine

    _assert_section_failure_is_visible(
        tmp_path, monkeypatch, caplog,
        (engine, "scan_slowdown"), "scan health",
    )
