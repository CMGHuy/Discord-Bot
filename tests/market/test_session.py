...
def _berlin(y, m, d, hh, mm):
    return dt.datetime(y, m, d, hh, mm, tzinfo=BERLIN_TZ)
...
@pytest.mark.parametrize(("moment", "expected"), [
    (_berlin(2026, 8, 27, 22, 59), False),
    (_berlin(2026, 8, 27, 23, 0), True),
    (_berlin(2026, 8, 27, 2, 0), True),
    (_berlin(2026, 8, 27, 7, 59), True),
    (_berlin(2026, 8, 27, 8, 0), False),
    (_berlin(2026, 8, 27, 12, 0), False),
    (_berlin(2026, 8, 27, 19, 30), False),
])
...
def test_every_hour_of_the_weekend_is_quiet(hour):
    assert is_quiet_hours(_berlin(2026, 8, 29, hour, 0)) is True   # Saturday
    assert is_quiet_hours(_berlin(2026, 8, 30, hour, 0)) is True   # Sunday


def test_quiet_hours_never_overlaps_the_regular_session():
    # The window (23:00-08:00 Berlin by default) sits entirely outside
    # 09:30-16:00 ET even after the +6h (August) ET->Berlin shift, on either
    # end, so a regular-session ET moment is never quiet.
...
def test_the_window_is_read_from_config(monkeypatch):
    ...
    monkeypatch.setattr(config, "QUIET_HOURS_START_BERLIN", 20)
    monkeypatch.setattr(config, "QUIET_HOURS_END_BERLIN", 6)
    assert is_quiet_hours(_berlin(2026, 8, 27, 19, 59)) is False
    assert is_quiet_hours(_berlin(2026, 8, 27, 20, 0)) is True
    assert is_quiet_hours(_berlin(2026, 8, 27, 5, 59)) is True
    assert is_quiet_hours(_berlin(2026, 8, 27, 6, 0)) is False


def test_a_utc_input_is_converted_before_the_window_is_applied():
    # 2026-08-27 20:00 UTC is 2026-08-27 22:00 Berlin (CEST, UTC+2) -- a
