...
CONFIDENCE_EXPLAINER = (
    ...
    f"**`!check` shows every scenario with a real entry point, even ones below Level {config.MIN_ALERT_CONFIDENCE_LEVEL} "
    "or short on other requirements** -- any unmet requirement (min reward %, stop distance, risk:reward, min "
    "strategies confirmed, min confidence) is marked in **bold red** rather than hidden, so it's always clear "
    "why a setup isn't a clean alert. The automatic background scan is stricter: it only ever posts a scenario "
    "once every requirement is met, to keep the alerts channel free of noise.\n\n"
